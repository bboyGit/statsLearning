...
def knn(trainX, trainY, testX, k):
    ...
    Return: An array with 2 columns. The final column is the class corresponding to that row and the remain are testX.
    Note:
...
if __name__ == "__main__":
    ...
    hat = knn(train_x, train_y, test_x, k=20)
    pred_real = np.concatenate([test_y, hat], axis=1)
    ...
    all(skhat == hat.ravel())    # 分类结果与 scikit-learn 一致
